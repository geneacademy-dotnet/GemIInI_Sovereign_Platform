...
export const normalizeGaId = (value) => {
    const match = String(value).trim().match(/^GA-?(\d{3,6})$/i);
 ...
 * Routes through the same remote proxy as `submitRegistration` when
 * configured (e.g. a Google Apps Script Web App endpoint) so the actual
 * GA-ID minting and sheet logging happen server-side, never in the
 * browser. Falls back to a PocketBase collection if no remote endpoint
 * is configured, matching the pattern above.
 */
export const submitBlsRegistration = async (payload) => {
    ...
    // No remote minter configured — GA-ID stays pending admin assignment,
    // same as the general registration flow.
