      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-12">
          <span className="inline-flex items-center gap-1.5 px-3.5 py-1 bg-[#00F2FE]/10 border border-[#00F2FE]/30 text-[#00F2FE] text-xs font-mono font-bold rounded-full uppercase tracking-wider">
            ...
            <span>{isRtl ? 'الركائز الأكاديمية السيادية' : 'SudaGene Academic Pillars'}</span>
          </span>
          <h2 className="text-3xl md:text-4xl font-extrabold text-white mt-3">
            {isRtl ? 'المسارات الأكاديمية والبرامج التخصصية' : 'Academic Tracks & Specialized Programs'}
          </h2>
          <p className="text-sm text-slate-400 mt-2 max-w-xl mx-auto">
            {isRtl
              ? 'برامج تدريبية معتمدة وفق إطار Mechanism-to-Clinic (MTC™) مع توثيق الساعات المعتمدة في السجل السيادي.'
              : 'Accredited training programs built on the Mechanism-to-Clinic (MTC™) framework with certified CPD hours recorded in the Sovereign Registry.'}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-[#0A0F1D] border border-slate-800 rounded-2xl p-6 flex flex-col justify-between hover:border-[#00F2FE]/50 transition-all shadow-xl group">
            <div>
              <div className="flex items-center justify-between mb-4">
                <span className="px-2.5 py-1 bg-teal-500/10 text-teal-400 text-xs font-bold rounded-lg border border-teal-500/30">
                  {isRtl ? 'Tier 1 • المسار السريري' : 'Tier 1 • Clinical'}
                </span>
              ...
              <p className="text-xs text-slate-400 leading-relaxed mb-4">
                {isRtl
                  ? 'التحضير لامتحانات المزاولة والزمالات الدولية (SMC, MRCS, USMLE, Approbation) عبر محاكاة الحالات السريرية المعقدة.'
              ...
              <ul className="space-y-2 text-xs text-slate-300 mb-6">
                <li className="flex items-center gap-2">
                  ...
                  <span>{isRtl ? '8 وحدات تدريبية وفق نموذج MTC™' : '8 Training modules aligned with MTC™'}</span>
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-teal-400 flex-shrink-0" />
                  <span>{isRtl ? 'بنك الأسئلة عالي الدقة (20Q & 40Q)' : 'High-yield question banks (20Q & 40Q blocks)'}</span>
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-teal-400 flex-shrink-0" />
                  <span>{isRtl ? 'دورة الإنعاش القلبي AHA BLS (دفعة 28 أغسطس)' : 'AHA-compliant BLS Certification Track'}</span>
                </li>
              </ul>
            </div>
            <Link
            ...
            >
              <span>{isRtl ? 'الانضمام للمسار السريري' : 'Join Clinical Track'}</span>
          ...
          <div className="bg-[#0A0F1D] border border-purple-500/40 rounded-2xl p-6 flex flex-col justify-between hover:border-purple-500 transition-all shadow-xl relative group">
            <div className="absolute -top-3 right-6 px-3 py-0.5 bg-purple-600 text-white text-[10px] font-bold rounded-full uppercase tracking-wider">
              {isRtl ? 'البحث الجزيئي' : 'Molecular Research'}
            </div>
            <div>
              <div className="flex items-center justify-between mb-4 mt-1">
                <span className="px-2.5 py-1 bg-purple-500/10 text-purple-300 text-xs font-bold rounded-lg border border-purple-500/30">
                  {isRtl ? 'Tier 2 • الجينوم' : 'Tier 2 • Genomics'}
                </span>
              ...
              <p className="text-xs text-slate-400 leading-relaxed mb-4">
                {isRtl
                  ? 'أبحاث الطب الجزيئي، تسلسل الجيل القادم (NGS)، وتحليل البيانات الجينومية ضمن خلايا النشر 15:5:1.'
                  : 'Translational oncology, Next-Generation Sequencing (NGS), and genomic pipelines in 15:5:1 publishing pods.'}
              </p>
              <ul className="space-y-2 text-xs text-slate-300 mb-6">
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-purple-400 flex-shrink-0" />
                  <span>{isRtl ? 'منهج MM 1.0–8.0 للطب الجزيئي' : 'MM 1.0–8.0 Molecular Medicine Series'}</span>
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-purple-400 flex-shrink-0" />
                  <span>{isRtl ? 'تحليل التنوع الجيني وعلم الأورام الدقيق' : 'Variant analysis & precision oncology ctDNA'}</span>
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-purple-400 flex-shrink-0" />
                  <span>{isRtl ? 'مشروع إنقاذ وتوثيق 100 ورقة بحثية' : 'The 100 Papers Project & Thesis Support'}</span>
                </li>
              </ul>
            </div>
            <Link
            ...
            >
              <span>{isRtl ? 'الانضمام للمسار الجزيئي' : 'Join Molecular Track'}</span>
              <ArrowIcon className="w-3.5 h-3.5" />
            </Link>
          </div>
          ...
          <div className="bg-[#0A0F1D] border border-slate-800 rounded-2xl p-6 flex flex-col justify-between hover:border-amber-500/50 transition-all shadow-xl group">
            <div>
              <div className="flex items-center justify-between mb-4">
                <span className="px-2.5 py-1 bg-amber-500/10 text-amber-400 text-xs font-bold rounded-lg border border-amber-500/30">
                  {isRtl ? 'Tier 3 • الإدارة والتشغيل' : 'Tier 3 • Operations & Management'}
                </span>
              ...
              <p className="text-xs text-slate-400 leading-relaxed mb-4">
                {isRtl
                  ? 'قوائم الحجز المبكر للمسارات القيادية واللوجستية: طب الطوارئ (EM-302)، منهجية البحث (RES-310)، وإدارة المستشفيات (MGT-400).'
              ...
              <ul className="space-y-2 text-xs text-slate-300 mb-6">
                <li className="flex items-center gap-2">
                  ...
                  <span>{isRtl ? 'حجز مقاعد مبكر مع بونص +250 GP' : 'Priority early-bird access with +250 GP bonus'}</span>
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-amber-400 flex-shrink-0" />
                  <span>{isRtl ? 'إدارة سلاسل إمداد الأجهزة والمستهلكات' : 'Turnkey diagnostic laboratory & reagent cold chain'}</span>
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-amber-400 flex-shrink-0" />
                  <span>{isRtl ? 'تأهيل القيادات الطبية والإدارية' : 'Executive clinical governance & practice management'}</span>
                </li>
              </ul>
            </div>
            <Link
            ...
            >
              <span>{isRtl ? 'تسجيل في قوائم الانتظار' : 'Reserve Early-Bird Seat'}</span>
              <ArrowIcon className="w-3.5 h-3.5" />
            </Link>
          </div>
