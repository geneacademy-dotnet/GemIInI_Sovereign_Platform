export const DATABASE_ROUTER_CONFIG = {
  "version": "2026.1",
  "system_name": "SudaGene Sovereign Database & Folder Routing Matrix",
  "vault_root": "G:\\My Drive\\01_SOVEREIGN_REGISTRY_VAULTS",
  "routes": [
    {
      "category": "pre_med_high_school",
      "program_code": "BWG-BIO",
      "track": "Biology with Gibbril (BWG) & High School Foundation",
      "target_sheet": "GA_MASTER_REGISTRY",
      "drive_workspace_folder": "GA-Workspaces/PreMed_Scholars",
      "assigned_pod": "Pod-Foundations",
      "gp_initial_bonus": 500,
      "tier": "Displaced Scholar",
      "access_level": "Level-1-Foundations"
    },
    {
      "category": "clinical_licensure_mbbs",
      "program_code": "SMC-101-108",
      "track": "GemIInI Academy — SMC Permanent Licensure Track",
      "target_sheet": "GA_MASTER_REGISTRY",
      "drive_workspace_folder": "GA-Workspaces/Clinical_Vanguard",
      "assigned_pod": "Pod-Clinical-SMC",
      "gp_initial_bonus": 500,
      "tier": "Clinical Vanguard",
      "access_level": "Level-5-Clinical"
    },
    {
      "category": "molecular_medicine_postgrad",
      "program_code": "MM-1.0-8.0",
      "track": "GeneAcademy — Molecular Medicine & WES Analytics",
      "target_sheet": "GA_MASTER_REGISTRY",
      "drive_workspace_folder": "GA-Workspaces/Molecular_Fellows",
      "assigned_pod": "Pod-1-Oncology",
      "gp_initial_bonus": 500,
      "tier": "Senior Molecular Fellow",
      "access_level": "Level-4-Genomics"
    },
    {
      "category": "hospital_b2b_infrastructure",
      "program_code": "GLOM-601-802",
      "track": "GLOMEt HQ — Hospital Partner & Lab Outfitting",
      "target_sheet": "GLOMET_B2B_PARTNER_REGISTRY",
      "drive_workspace_folder": "GLOMEt-Workspaces/Partner_Facilities",
      "assigned_pod": "B2B-Telemetry-Node",
      "gp_initial_bonus": 2500,
      "tier": "Institutional Partner",
      "access_level": "Level-5-Hardware"
    }
  ],
  "university_cluster_mapping": {
    "UofK": "Cluster-Central-Khartoum",
    "Gezira": "Cluster-Central-Gezira",
    "Nile": "Cluster-Private-Excellence",
    "National": "Cluster-Clinical-HighVolume",
    "Ahfad": "Cluster-Community-Maternal",
    "RedSea": "Cluster-Coastal-Epidemiology",
    "Kordofan": "Cluster-Western-Regional"
  }
};
